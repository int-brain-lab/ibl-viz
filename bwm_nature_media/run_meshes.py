...
def create_transparent_material():
    ...
    if bsdf:
        bsdf.inputs["Alpha"].default_value = 0.5  # 10% opacity
...
for mesh in meshes:
    ...
    if imported_obj and imported_obj.type == 'MESH':
        ...
        imported_obj.location = offset_origin
        imported_obj.scale = (scale_factor, scale_factor, scale_factor)
        # reset the origin to the blender 0,0,0 point


        color = data[str(mesh)]

        # copy the material and change the color to the one from the dataframe
        new_mat = transparent_mat.copy()
        new_mat.name = f"TransparentMaterial_{mesh}"
        new_mat.node_tree.nodes["Principled BSDF"].inputs["Base Color"].default_value = color

        # Assign transparent material
        if len(imported_obj.data.materials) == 0:
            imported_obj.data.materials.append(new_mat)
        else:
            imported_obj.data.materials[0] = new_mat
